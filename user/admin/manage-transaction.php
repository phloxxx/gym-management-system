                    ...
                    <div>
                        ...
                        <div class="relative rounded-md shadow-sm">
                            ...
                            <select id="subFilter" class="pl-10 w-full px-4 py-2.5 border border-gray-300 rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary-light focus:border-transparent transition-all duration-200 appearance-none bg-white">
                                ...
                                <?php foreach ($subscriptionPlans as $sub): ?>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function initActionButtons() {
                ...
                document.querySelectorAll('[data-action="deactivate"]').forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        console.log("Deactivate button clicked with:", { subId, memberId });
                        
                        // Validate that we have both IDs
                        if (!subId || !memberId) {
                            console.error("Missing required data attributes:", { subId, memberId });
                        ...
                        showConfirmationDialog(
                            ...
                            function() {
                                ...
                                if (row) {
                                    ...
                                    showToast('Error: Could not find subscription details', false);
                                }
                            }
                        );
                    });
            ...
            function deactivateSubscription(memberId, subId, row) {
                ...
                console.log('Deactivating subscription:', { memberId, subId });
...
<!-- Add a new script tag specifically for handling deactivation -->
<script>
    // Fix for subscription deactivation
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deactivateButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                const subId = this.getAttribute('data-sub-id');
                const memberId = this.getAttribute('data-member-id');
                console.log('Deactivate button clicked for subId:', subId, 'memberId:', memberId);
                ...
                showConfirmationDialog(
                    ...
                    () => {
                        ...
                        // Make the API call to deactivate-subscription.php
                        fetch('../../functions/deactivate-subscription.php', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                // Update the UI to show inactive status
                                const statusCell = row.querySelector('td:nth-child(6) span');
                                statusCell.className = 'px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800';
                                statusCell.textContent = 'Inactive';
                                ...
                                // Re-attach event listeners to new renew buttons
                                const renewButton = actionCell.querySelector('[data-action="renew"]');
                                if (renewButton) {
                                    renewButton.addEventListener('click', function() {
                                        const memberId = this.getAttribute('data-member-id');
                                        const subId = this.getAttribute('data-sub-id');
        ...
        // Also fix any renew buttons that might already be present
        const renewButtons = document.querySelectorAll('[data-action="renew"]');
        console.log('Found renew buttons:', renewButtons.length);
